...
# Le package ggmisc permet facilement d'afficher l'équation du modèle linéaire ainsi que des métrique d'erreur
ma_formule = y ~ x -1
#ma_formule = y ~ x
...
**Exercice 3**
Reproduisez le scatterplot ci-dessous entre hwy et displ puis entre hwy et cyl; entre class et drv.
```{r,echo=FALSE}
ggplot(data = mpg) + 
...
### Dessiner de l'Esthétique gaphique
...
```{r,echo=FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy,color=(class=="2seater"|(class=="midsize"&displ>5))))+
 ...
 * Que faut il ajouter pour corriger le graphique avec ? ([indice](http://www.cookbook-r.com/Graphs/Shapes_and_line_types/) et regarder le message de warning afficher en console)
...
L'esthétique `group` permet de dessiner plusieurs ligne en fonction d'une colonne de la data frame.
...
```
* Refaire ces graphes ci-dessous : 
...
** Exercice 4**
 ...
 1. Quel l'équivalent géométrique de `stat_summary()` ? Modifiez le code ci-dessus en utilisant une fonction `geom_`.
...
1. Dessiner le département et une région de votre choix ?
